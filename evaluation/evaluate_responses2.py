...
responses_df = pd.read_csv("survey_responses_with_ids2.csv")


def precompute_game_features(game_db_path="game_database.csv"):
    last_week_file = "cached_features_last_week1.pkl"
    liked_range_file = "cached_features_liked_range1.pkl"
    ...
    games_df['date'] = pd.to_datetime(games_df['date'], format="mixed", dayfirst=True ,errors='coerce')

    last_week_start = datetime(2025, 5, 11)
    last_week_end = datetime(2025, 5, 17)
    liked_range_start = datetime(2025, 4, 6)
    liked_range_end = datetime(2025, 5, 10)
...
def run_evaluation():
    ...
    for index, row in responses_df.iterrows():
        ...
        R = len(relevant_ids)
        # recommendations = recommend_games_cosine_max(all_games, liked_games, num=11)
        recommendations = recommend_games_random(all_games, 11)
        # recommendations = recommend_games_cosine(all_games, [], num=11)
    ...
    # Compute mean of all numeric columns except user_id
    mean_row = result_df.drop(columns=["user_id"]).mean(numeric_only=True)
    mean_row["user_id"] = "mean"  # Label row

    # Append mean row to dataframe
    result_df = pd.concat([result_df, pd.DataFrame([mean_row])], ignore_index=True)

    result_df.to_csv("evaluation_metrics_rnd2.csv", index=False)
    print("\n✅ Evaluation complete. Metrics saved to 'evaluation_metrics2.csv'.")
...
def run_evaluation_varying_k():
    ...
    for k in k_values:
        ...
        for index, row in responses_df.iterrows():
            ...
            R = len(relevant_ids)
            recommendations = recommend_games_cosine_top_k(all_games, liked_games, num=11, k=k)
            recommended_ids = [g['game_id'] for g in recommendations]
    ...
    # Save results
    pd.DataFrame(summary).to_csv("evaluation_by_k2.csv", index=False)
